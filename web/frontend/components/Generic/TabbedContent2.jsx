...
export const TabbedContent2 = ({
  ...
  const alltabpanels = Array(selectedTabIndex + 1).fill(null)?.map((_, index) => {
    const key = `.${index+1}`;
    const findIfComponentExists = tabPanelChildComponents?.find(child => child.key === key);
    if(findIfComponentExists){
      return findIfComponentExists;
    }
    return null;
  });

  console.log('renderedTabs2',alltabpanels,tabPanelChildComponents?.map(child => child.key),hasTabSectionChild, selectedTabIndex);
  ...
  return (
    ...
    >
      ...
      <TabPanels mt={4}>
        {(alltabpanels || tabPanelChildComponents)?.map((child, index) => {
          return (
